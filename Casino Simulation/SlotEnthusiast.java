    ...
    public void act()
    {
        if(!playing) {
            ...
            if(Math.abs(tx-getX())>2){
                ...
                if(!alreadyPlaced){
                    if(SpotManager.getGames()[going.getGameIndex()].isSomeonePlaying(going.getSpotIndex())&&SpotManager.getGames()[going.getGameIndex()].isSpotTaken(going.getSpotIndex()))playDialogue((Greenfoot.getRandomNumber(2)==0?"Get out of the way.":"I'm rich. Got a problem?"));
                    SpotManager.getGames()[going.getGameIndex()].absolutePlaceGambler(this,going.getSpotIndex());
                }
                playing=true;
